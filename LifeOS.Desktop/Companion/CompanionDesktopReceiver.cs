...
public sealed class CompanionDesktopReceiver : IDisposable
{
 ...
 { try { if(ctx.Request.HttpMethod=="GET"&&ctx.Request.Url?.AbsolutePath=="/glance"){await GlanceAsync(ctx);return;} if(ctx.Request.HttpMethod!="POST"){ctx.Response.StatusCode=405;return;} var body=await new StreamReader(ctx.Request.InputStream,Encoding.UTF8).ReadToEndAsync(ct);
 ...
 { var payload=JsonSerializer.Deserialize<CaptureTransfer>(body,new JsonSerializerOptions(JsonSerializerDefaults.Web));if(payload is null||payload.ProtocolVersion!=CompanionProtocol.Version||payload.Body.Length>20000||payload.Title?.Length>200||payload.Category?.Length>80){await Write(ctx,new TransferAcknowledgement(CompanionProtocol.Version,payload?.TransferId??"",payload?.CaptureId??"","",DateTimeOffset.UtcNow,TransferResult.Invalid));return;}
   ...
   var intakes=_store.LoadIntakes();var existing=intakes.FirstOrDefault(x=>x.IdempotencyKey==payload.IdempotencyKey||x.CaptureId==payload.CaptureId);if(existing is not null){var result=existing.CaptureId==payload.CaptureId&&existing.ContentHash!=payload.ContentHash?TransferResult.Conflict:TransferResult.Duplicate;await Write(ctx,new TransferAcknowledgement(CompanionProtocol.Version,payload.TransferId,payload.CaptureId,existing.IntakeId,existing.ReceivedAtUtc,result));return;}
   var intake=new MobileCompanionIntake(Guid.NewGuid().ToString("N"),payload.CaptureId,pairing.DeviceLabel,payload.TransferId,payload.IdempotencyKey,payload.ContentHash,payload.Title,payload.Body,payload.Category,payload.CreatedAtUtc,DateTimeOffset.UtcNow,IntakeReviewState.NeedsReview,$"Mobile Companion - {pairing.DeviceLabel} - protocol {payload.ProtocolVersion}");intakes.Add(intake);_store.SaveIntakes(intakes);
 ...
 private async Task GlanceAsync(HttpListenerContext ctx)
 {var pairingId=ctx.Request.Headers["X-LifeOS-Pairing"]??"";var ts=ctx.Request.Headers["X-LifeOS-Timestamp"]??"";var nonce=ctx.Request.Headers["X-LifeOS-Nonce"]??"";var sig=ctx.Request.Headers["X-LifeOS-Signature"]??"";var pairing=_store.LoadPairings().FirstOrDefault(x=>x.PairingId==pairingId&&!x.Revoked);if(pairing is null||!long.TryParse(ts,out var seconds)||Math.Abs(DateTimeOffset.UtcNow.ToUnixTimeSeconds()-seconds)>120||string.IsNullOrWhiteSpace(nonce)||!_nonces.Add(nonce)||!CompanionAuthenticator.Verify(pairing.SecretBase64,ts,nonce,"",sig)){ctx.Response.StatusCode=401;return;}var now=DateTimeOffset.UtcNow;var needsReview=_store.LoadIntakes().Count(x=>x.ReviewState==IntakeReviewState.NeedsReview);var snapshots=new[]{new LifeOS.Companion.Core.Models.GlanceSnapshot(LifeOS.Companion.Core.Models.GlanceKind.Agenda,"LifeOS Desktop",now,now.AddHours(6),new[]{new LifeOS.Companion.Core.Models.GlanceItem("agenda-status","Open Desktop Agenda for trusted detail","Companion cache is deliberately compact",null,"LifeOS Desktop")}),new LifeOS.Companion.Core.Models.GlanceSnapshot(LifeOS.Companion.Core.Models.GlanceKind.WaitingOnWork,"LifeOS Desktop",now,now.AddHours(6),new[]{new LifeOS.Companion.Core.Models.GlanceItem("review-count",$"{needsReview} mobile review item(s)","Read-only summary; no module mutation",null,"LifeOS Desktop")})};await Write(ctx,new GlanceResponse(CompanionProtocol.Version,snapshots));}
