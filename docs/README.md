# LifeOS Documentation

## Authoritative current documents

- `current-status.md` â€” current completed Group 25 state and safety boundary.
- `lifeos-version-state.json` â€” machine-readable `v5.0.0-alpha.5` state.
- `version-history.md` â€” consolidated milestone history.
- `release-notes/v5.0-alpha-group-25.md` â€” Group 25 release record.
- `screenshot-groups/group-25-read-only-gmail-connector/README.md` â€” current screenshot evidence.
- `screenshot-groups/group-24-email-radar-foundation/README.md` â€” prior Email Radar foundation evidence.
- `integrations/gmail-connector-setup.md` â€” private/testing-mode connector setup.
- `integrations/v5-integration-contract.md` â€” connector safety and handoff contract.

## Screenshot groups

Groups 01â€“09 cover the v1â€“v3.9 offline operating-system foundation.

Groups 10â€“19 cover the complete v4 spine.

The v5 lane currently includes:

- Group 20 â€” Manual Import Connector
- Group 21 â€” v5.0-alpha Alignment
- Group 22 â€” Google Calendar Read-Only Connector
- Group 23 â€” Connector Lifecycle Hardening
- Group 24 â€” Email Radar Foundation
- Group 25 â€” Authenticated Read-Only Gmail Connector

Group 25 is the current completed screenshot boundary. It proves exact read-only scope, explicit browser connection, manual profile-bound bounded search, visible query/exclusions, retrieval confirmation, safe empty and successful results, provider-neutral evidence intake, confirmed-only timeline, repeated-search duplicate detection, disconnect, cache clear, and retained imported evidence.

Group 26 has not started.

## Documentation rule

Current-state documents describe only the current product state. Historical release notes and screenshot groups remain unchanged as evidence. Credentials, local token caches, private communication data, generated backups, and temporary screenshot plans must not be committed.

- [Group 26 — v5 integration release checkpoint](docs/screenshot-groups/group-26-v5-integration-release-checkpoint/README.md)
