...
public sealed class OutboxPage : ContentPage
{
    ...
    private async Task RefreshAsync()
    {
        ...
        { var state=new Label {Text=$"State: {capture.DeliveryState}"}; var send=new Button {Text=capture.DeliveryState==DeliveryState.Failed?"Retry manually":capture.DeliveryState==DeliveryState.Delivered?"Resend duplicate proof":"Send manually",IsEnabled=capture.DeliveryState!=DeliveryState.Sending};
    ...
    private async Task SendAsync(QuickCapture capture, Label state, Button button)
    {
        ...
        try { button.IsEnabled=false; await _store.UpdateDeliveryStateAsync(capture.CaptureId,DeliveryState.Sending); state.Text="State: Sending"; var ack=await _client.SendAsync(credential,capture);
          if(ack.CaptureId==capture.CaptureId && ack.Result==TransferResult.Conflict){await _store.SaveConflictAsync(new CaptureConflict(Guid.NewGuid().ToString("N"),capture.CaptureId,capture.ContentHash,"desktop-version",DateTimeOffset.UtcNow,false,null,null));await _store.UpdateDeliveryStateAsync(capture.CaptureId,DeliveryState.Failed);state.Text="State: Failed — changed-content conflict requires explicit resolution";}
          else if(ack.CaptureId==capture.CaptureId && ack.Result is TransferResult.AcceptedForReview or TransferResult.Duplicate){await _store.UpdateDeliveryStateAsync(capture.CaptureId,DeliveryState.Delivered);state.Text=$"State: Delivered ({ack.Result})";}
