...
public sealed class MobileFoundationService
{
    ...
    public MobileFoundationService(IMobileLocalStore store)
    {
        _store = store;
    }
    ...
    public Task<MobileSyncSnapshot> GetSyncSnapshotAsync(
        CancellationToken cancellationToken = default) =>
        _store.LoadSyncAsync(cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _store.SaveSyncAsync(
            current with
            {
                State = MobileSyncState.Stopped,
                SafeError = null
            },
            cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await _store.SaveSyncAsync(
            current with
            {
                State = current.PendingCount > 0
                    ? MobileSyncState.Pending
                    : MobileSyncState.Idle
            },
            cancellationToken);
    }

    public async Task<IReadOnlyList<MobileDiagnosticItem>>
        GetSafeDiagnosticsAsync(
            CancellationToken cancellationToken = default)
    {
        ...
        [
            ...
            new("Home mode", "Daily overview • review-first"),
            new("Provider writes", "Disabled"),
            new("Sensitive values", "Redacted")
        ];
    }

    public static string DeterministicId(string value) =>
        Convert
            .ToHexString(
                SHA256.HashData(
