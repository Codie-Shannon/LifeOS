...
{
    private readonly V11MoneyService _service = new(); private readonly VerticalStackLayout _content = new();
    public MoneyPage(){ Title="Money"; BackgroundColor=MoneyVisuals.Background; Content=new ScrollView{Content=_content}; }
    protected override void OnAppearing(){base.OnAppearing(); Render();}
    private void Render(){ var now=DateTimeOffset.Now; var overview=_service.BuildOverview(now); var data=_service.BuildProofData(now); _content.Children.Clear(); _content.Padding=new Thickness(18,14,18,30); _content.Spacing=13;
      ...
      _content.Children.Add(MoneyVisuals.CardView("Boundary","No bank feed, payment initiation, provider write, accounting approval or autonomous reconciliation exists.")); }
    private void AddAction(string text, Func<Task> action){var b=MoneyVisuals.Action(text,Color.FromArgb("#2C3140")); b.Clicked+=async(_,_)=>await action(); _content.Children.Add(b);}
}
