...
$ScenarioPath = Join-Path $Repository 'tools\evidence\group-45-scenarios.json'
$EvidenceFolder = Join-Path $Repository 'docs\screenshot-groups\group-45-v8-release'

if (-not (Test-Path -LiteralPath $ScenarioPath -PathType Leaf)) {
    throw "Group 45 scenario manifest not found: $ScenarioPath"
}

$Manifest = Get-Content -LiteralPath $ScenarioPath -Raw | ConvertFrom-Json

if ([int]$Manifest.group -ne 45) {
    throw "Unexpected evidence group: $($Manifest.group)"
}

if ([string]$Manifest.release -ne 'v8.0.0-beta.1') {
...
if (@($Ids | Sort-Object -Unique).Count -ne 8) {
    throw 'Group 45 scenario IDs are not unique.'
}

if (@($Files | Sort-Object -Unique).Count -ne 8) {
    throw 'Group 45 screenshot names are not unique.'
}
...
if ($RequireImages) {
    ...
    if ($Missing.Count -gt 0) {
        throw "Missing Group 45 screenshots:`n$($Missing -join "`n")"
    }
    ...
    if ($UnexpectedPng.Count -gt 0) {
        throw "Unexpected Group 45 PNG files:`n$($UnexpectedPng.FullName -join "`n")"
    }
}

Write-Host 'GROUP 45 EVIDENCE CONTRACT PASS' -ForegroundColor Green
Write-Host 'Scenarios: 8/8 unique and ordered' -ForegroundColor Green
Write-Host 'Release: v8.0.0-beta.1' -ForegroundColor Green
